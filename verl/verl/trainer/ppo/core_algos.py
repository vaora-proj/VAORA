...
def LLDS_penalty(log_prob: torch.Tensor, 
                 old_log_prob: torch.Tensor, 
                 response_mask: torch.Tensor) -> torch.Tensor:
    """
    Computes Lazy Likelihood Displacement with Response-Level Gating (LLDS).
    
    Equation:
    L_LLDS = Gate * Sum(max(0, log_pi_old - log_pi_new))
    Where Gate = 1 if Sum(log_pi_old - log_pi_new) > 0 else 0
    """
    # 1. Calculate Displacement: log(pi_old) - log(pi_new)
    # Positive displacement means the model became LESS likely (worse) on that token.
    displacement = old_log_prob - log_prob
    
    # 2. Apply Mask: Ensure padding tokens are strictly 0
    displacement = displacement * response_mask
    
    # 3. Response-Level Gating
    # Check if the TOTAL likelihood of the response decreased.
    # Sum displacement across sequence dimension (dim=-1)
    response_displacement_sum = displacement.sum(dim=-1)
    
    # Gate is 1 (Active) if total displacement > 0 (Response got worse overall)
    # Gate is 0 (Inactive) if total displacement <= 0 (Response improved or same)
    gate = (response_displacement_sum > 0).float()
    
    # 4. Token-Level Penalty (LLD logic)
    # We only penalize tokens that specifically got worse (displacement > 0)
    token_penalty = torch.nn.functional.relu(displacement)
    
    # 5. Apply Gate to Penalty
    # Broadcast gate (bsz,) -> (bsz, 1) to multiply with token_penalty (bsz, seq_len)
    # If gate is 0, this zeros out the penalty for the entire sequence.
    llds_loss_mat = token_penalty * gate.unsqueeze(-1)
    
    return llds_loss_mat
