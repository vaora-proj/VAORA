...
def compute_reward(data: DataProto, reward_fn: AbstractRewardManager) -> tuple[torch.Tensor, dict[str, Any]]:
    ...
    """
    print(f"[compute_reward] reward_fn: {reward_fn.__class__.__name__}")
    try:
        reward_result = reward_fn(data, return_dict=True)
        score_tensor = reward_result["score_tensor"]
        placement_tensor = reward_result["placement_tensor"]
        collision_tensor = reward_result["collision_tensor"]
        grounding_tensor = reward_result["grounding_tensor"]
        prob_tensor = reward_result["prob_tensor"]
        reward_extra_infos_dict = reward_result.get("reward_extra_info", {})
        print(f"[compute_reward] score_tensor: {score_tensor.shape}")
        print(f"[compute_reward] placement_tensor: {placement_tensor.shape}")
        print(f"[compute_reward] collision_tensor: {collision_tensor.shape}")
        print(f"[compute_reward] grounding_tensor: {grounding_tensor.shape}")
        print(f"[compute_reward] prob_tensor: {prob_tensor.shape}")
    ...
    return score_tensor, placement_tensor, collision_tensor, grounding_tensor, prob_tensor, reward_extra_infos_dict
