...
from verl.trainer.ppo.core_algos import agg_loss, get_policy_loss_fn, kl_penalty, LLDS_penalty
            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if self.config.use_llds_loss:
                        # TODO
                        # 1. Compute the per-token gated penalty matrix
                        llds_loss_mat = LLDS_penalty(
                            log_prob=log_prob, 
                            old_log_prob=old_log_prob, 
                            response_mask=response_mask
                        )

                        # 2. Aggregate the loss (normalize by valid tokens or batch size)
                        # We reuse the existing agg_loss helper for consistency
                        llds_loss = agg_loss(
                            loss_mat=llds_loss_mat, 
                            loss_mask=response_mask, 
                            loss_agg_mode=loss_agg_mode
                        )

                        # 3. Add to total policy loss
                        # Assuming 'llds_coef' is defined in config (e.g., 1.0 or 0.1)
                        # If not in config, replace with a hardcoded float or default.
                        llds_coef = getattr(self.config, "llds_coef", 0.1)
                        policy_loss = policy_loss + llds_loss * llds_coef

                        # 4. Update Metrics for logging
                        metrics["actor/llds_loss"] += llds_loss.detach().item() * loss_scale_factor
                        micro_batch_metrics["actor/llds_coef"] = llds_coef
