...
def prepare_single_generation_data(batch_dict, config) -> DataProto:
    ...
    full_batch.pop(
        batch_keys=[k for k in batch_keys_to_pop if k in full_batch.batch.keys()],
        non_tensor_batch_keys=[k for k in non_tensor_batch_keys_to_pop if k in full_batch.non_tensor_batch.keys()],
    )
