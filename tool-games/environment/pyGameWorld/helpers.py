...
__all__ = ['areaForSegment','areaForPoly','centroidForPoly','recenterPoly','objectComplexity',
           'segs2Poly','polyValidate', 'word2Color', 'applyColorRemap', 'distanceToObject','objectBoundingBox',
...
_BASE_COLOR_BY_NAME = {
    'blue': (24, 119, 242, 255),
    'red': (243, 79, 70, 255),
    'green': (107, 206, 187, 255),
    'black': (0, 0, 0, 255),
    'white': (255, 255, 255, 255),
    'grey': (127, 127, 127, 255),
    'lightgrey': (185, 202, 210, 255),
    'none': (0, 0, 0, 0),
}

# Global color remap requested for tool-games:
...
_BASE_COLOR_REMAP = {
    _BASE_COLOR_BY_NAME[src]: _BASE_COLOR_BY_NAME[dst]
    for src, dst in _COLOR_NAME_REMAP.items()
}


def _to_rgba_tuple(color):
    vals = [int(c) for c in color]
    if len(vals) == 3:
        vals.append(255)
    if len(vals) != 4:
        raise Exception('Color vector must be RGB or RGBA')
    return tuple(vals)


def applyColorRemap(color):
    if color is None:
        return None
    if isinstance(color, str):
        cname = color.lower()
        cname = _COLOR_NAME_REMAP.get(cname, cname)
        if cname not in _BASE_COLOR_BY_NAME:
            raise Exception('Color name not known: ' + cname)
        return _BASE_COLOR_BY_NAME[cname]

    rgba = _to_rgba_tuple(color)
    return _BASE_COLOR_REMAP.get(rgba, rgba)
...
def word2Color(colorname):
    if colorname is None:
        return None
    try:
        # Parse numeric RGB/RGBA vectors only; remapping is applied later
        # in object constructors so it happens exactly once globally.
        return _to_rgba_tuple(colorname)
    except:
        c = colorname.lower()
        if c not in _BASE_COLOR_BY_NAME:
            raise Exception('Color name not known: ' + c)
        # Return canonical base color for named strings. Do not remap here
        # to avoid double-remapping during object construction.
        return _BASE_COLOR_BY_NAME[c]
